...
export function RenderUploadedState({ previewUrl, isDeleting, handleRemoveFile }: { previewUrl: string; isDeleting: boolean; handleRemoveFile: () => void }) {
  return (
    <div>
      ...
      <Button variant="destructive" size="icon" className={cn("absolute top-4 right-4")} type="button" onClick={handleRemoveFile} disabled={isDeleting} >
        { isDeleting ?(
...
export function RenderUploadingState({ previewUrl, progress }: { previewUrl: string; progress: number }) {
  return (
    <div className="relative w-full h-full">
      <Image src={previewUrl} alt="Uploading File" fill className="object-contain p-2 opacity-50" />
